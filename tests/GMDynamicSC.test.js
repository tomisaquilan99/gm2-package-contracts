...
describe("GMDynamicSC", () => {
  ...
  const defaultImagePath = "ipfs://default/image/path/";
  ...
  beforeEach(async () => {
    ...
    const DynamicMetadata = await ethers.getContractFactory(
      "DynamicMetadata",
      owner
    );
    ...
    gMDynamicSC = await GMDynamicSC.deploy(
      defaultBaseURI,
  ...
  describe("Default behavior", async () => {
    describe("setBaseAttributes", () => {
      ...
      it("Should be reverted if base attributes is set twice", async () => {
        await gMDynamicSC.setBaseAttributes(1, baseAttributes);
        await expect(
          gMDynamicSC.setBaseAttributes(1, baseAttributes)
        ).to.be.revertedWith("AlreadyHaveBaseAttributes");
        expect(true).to.be.true;
      });
    });
    ...
    describe("setImagePath", () => {
      it("Should be reverted if base attributes was not set before", async () => {
        await expect(gMDynamicSC.setImagePath(1, "newPath")).to.be.revertedWith(
          "CannotSetImageWithoutBaseAttributes"
        );
      });
      it("Should be set image path to token if it has base attributes", async () => {
        await gMDynamicSC.setBaseAttributes(1, baseAttributes);
        await gMDynamicSC.setImagePath(1, "newPath");
        expect(true).to.be.true;
      });
    });
    ...
    describe("canTokenBeTransferred", () => {
      it("Should returns true by default", async () => {
        expect(await gMDynamicSC.canTokenBeTransferred(gMDynamicSC.address, 1))
          .to.be.true;
      });
    });
    describe("tokenURI", () => {
    ...
    describe("transferBlockedToken", () => {
      it("Should be reverted if pass invalid address", async () => {
        await expect(
          gMDynamicSC.transferBlockedToken(ethers.constants.AddressZero, 1)
        ).to.be.revertedWith("ZeroAddressNotSupported");
      });
      it("Should be reverted by default", async () => {
        await expect(
          gMDynamicSC.transferBlockedToken(otherAccount.address, 1)
        ).to.be.revertedWith("MethodNotSupported");
      });
    });
    describe("setAttributesControllerSC", () => {
      ...
      beforeEach(async () => {
        const TestContractERC165WithoutIGMController =
          await ethers.getContractFactory("ERC721", owner);
        contractERC165WithoutIGMController = await deployMockContract(
          owner,
          TestContractERC165WithoutIGMController.interface.format(
            ethers.utils.FormatTypes.json
          )
        );
        await contractERC165WithoutIGMController.mock.supportsInterface.returns(
          false
        );

        GMAttributesController = await ethers.getContractFactory(
      ...
      it("Should be reverted if defaultImagePath was not set", async () => {
        await expect(
          gMDynamicSC.setAttributesControllerSC(gMAttributesController.address)
        ).to.be.revertedWith("DefaultImagePathRequired");
      });
      it("Should be reverted if passed address is not valid", async () => {
        gMDynamicSC.setDefaultImagePath(defaultImagePath);
        await expect(
          gMDynamicSC.setAttributesControllerSC(ethers.constants.AddressZero)
        ).to.be.revertedWith("ZeroAddressNotSupported");
      });
      it("Should be reverted if passed contract that not implements IERC165", async () => {
        gMDynamicSC.setDefaultImagePath(defaultImagePath);
        await expect(
          gMDynamicSC.setAttributesControllerSC(gMDynamicSC.address)
        ).to.be.revertedWith(
      ...
      it("Should be reverted if passed contract that implements IERC165 but not IGMAttributesController", async () => {
        gMDynamicSC.setDefaultImagePath(defaultImagePath);
        await expect(
          gMDynamicSC.setAttributesControllerSC(
            contractERC165WithoutIGMController.address
          )
        ).to.be.revertedWith("InterfaceIsNotImplemented");
      });
      it("Should set attributes controller address", async () => {
        gMDynamicSC.setDefaultImagePath(defaultImagePath);
        await gMDynamicSC.setAttributesControllerSC(
    ...
    describe("setTransferControllerSC", () => {
      ...
      before(async () => {
        ...
        GMTransferController = await ethers.getContractFactory(
      ...
      it("Should be reverted if passed address is not valid", async () => {
        await expect(
          gMDynamicSC.setTransferControllerSC(ethers.constants.AddressZero)
        ).to.be.revertedWith("ZeroAddressNotSupported");
      });
      it("Should be reverted if passed contract that not implements IERC165", async () => {
        await expect(
          gMDynamicSC.setTransferControllerSC(gMDynamicSC.address)
        ).to.be.revertedWith(
          "function selector was not recognized and there's no fallback function"
        );
      });
      it("Should be reverted if passed contract that implements IERC165 but not IGMAttributesController", async () => {
        await expect(
          gMDynamicSC.setTransferControllerSC(
            contractERC165WithoutIGMController.address
          )
        ).to.be.revertedWith("InterfaceIsNotImplemented");
      });
  ...
  describe("Dynamic behavior", async () => {
    ...
    describe("tokenURI", () => {
      beforeEach(async () => {
        GMAttributesController = await ethers.getContractFactory(
          "TestGMAttributesController",
          owner
        );
        gMAttributesController = await GMAttributesController.deploy();
        await gMDynamicSC.setDefaultImagePath(defaultImagePath);
        await gMDynamicSC.setAttributesControllerSC(
          gMAttributesController.address
        );
      });
    ...
    describe("transferBlockedToken", () => {
      ...
      it("Should return false if transfer is blocked", async () => {
        expect(await gMDynamicSC.canTokenBeTransferred(otherAccount.address, 1))
          .to.be.false;
      });
      it("Should return true if transfer is blocked", async () => {
        expect(await gMDynamicSC.canTokenBeTransferred(otherAccount.address, 2))
          .to.true;
      });

      it("Should transfer token is not blocked", async () => {
        await gMDynamicSC.transferBlockedToken(otherAccount.address, 2);
        expect(await gMDynamicSC.wasTransferred(otherAccount.address, 2)).to.be
          .true;
      });
    });
  });
